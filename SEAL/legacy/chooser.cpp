...
namespace seal
{
    ...
    bool ChooserEvaluator::select_parameters(const std::vector<ChooserPoly> &operands, int budget_gap, double noise_standard_deviation, double noise_max_deviation, const std::map<int, BigUInt> &parameter_options, EncryptionParameters &destination)
    {
        ...
        if (largest_bit_count >= 63)
        {
            // plain_modulus restricted to 63 bits
            return false;
        }
        else if (largest_bit_count == 62)
        {
            // Barrett ratio cannot be compute for 2^62, so set value to 2^62 + 1
            new_plain_modulus = (1ULL << largest_bit_count) + 1;
        }
        else
        {
        ...
        while (iter != parameter_options.end() && !found_good_parms)
        {
            ...
            if (dimension > largest_coeff_count && iter->second > *destination.plain_modulus().pointer() )
            {
                // Set the polynomial
    ...
    void ChooserEncoder::encode(uint64_t value, ChooserPoly &destination)
    {
        ...
        destination.reset();
        destination.max_coeff_count() = value_poly.get_poly().significant_coeff_count();
        destination.max_abs_value() = poly_infty_norm_coeffmod(value_poly.get_poly(), 
            BigUInt(encoder_.plain_modulus().bit_count(), *encoder_.plain_modulus().pointer()));
    }
    ...
    void ChooserEncoder::encode(int64_t value, ChooserPoly &destination)
    {
        ...
        encoder_.encode(value, value_poly);
        destination.reset();
        destination.max_coeff_count() = max(value_poly.get_poly().significant_coeff_count(), 1);
        destination.max_abs_value() = poly_infty_norm_coeffmod(value_poly.get_poly(), 
            BigUInt(encoder_.plain_modulus().bit_count(), *encoder_.plain_modulus().pointer()));
    }
