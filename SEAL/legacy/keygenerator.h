...
namespace seal
{
    ...
    Constructing an KeyGenerator requires the encryption parameters (set through a SEALContext object). 
    ...
    @warning KeyGenerator is not thread-safe and a separate instance is needed for each potentially 
    concurrent usage.
    */
    class KeyGenerator
    {
    public:
        /**
        Creates an KeyGenerator instance initialized with the specified SEALContext. Optionally, the 
        ...
        */
        KeyGenerator(const SEALContext &context, const MemoryPoolHandle &pool = MemoryPoolHandle::Global());

        /**
        Creates an KeyGenerator instance initialized with the specified SEALContext and specified
        ...
        @see MemoryPoolHandle for more details on memory pool handles.
        */
        KeyGenerator(const SEALContext &context, const SecretKey &secret_key, const PublicKey &public_key, 
            const EvaluationKeys &evaluation_keys, const MemoryPoolHandle &pool = MemoryPoolHandle::Global());
        ...
        */
        ...
        /**
        Generates evaluation keys so that there are count many in total. Each key is added as a new 
        entry to the std::vector of evaluation keys. This function is automatically called by generate() 
        to generate evaluation keys, but can be later called by the user to increase the number of 
        evaluation keys on top of what has already been generated. An error is thrown if the user tries 
        to generate evaluation keys before a secret key and public key have been generated.
    ...
    private:
        ...
        util::Pointer coeff_modulus_minus_one_;
        ...
        std::vector<util::Pointer> evaluation_factors_;
