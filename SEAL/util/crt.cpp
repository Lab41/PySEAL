...
namespace seal
{
    namespace util
    {
        UIntCRTBuilder::UIntCRTBuilder() : generated_(false), mod_array_alloc_(nullptr)
        {
        }

        UIntCRTBuilder::UIntCRTBuilder(vector<const uint64_t*> mods, vector<int> mod_uint64_counts)
        {
        ...
        bool UIntCRTBuilder::generate(vector<const uint64_t*> mods, vector<int> mod_uint64_counts)
        {
            ...
            mod_array_alloc_ = new uint64_t[mod_uint64_counts_sum];
            mod_product_ = allocate_uint(mod_product_uint64_count_, pool);
            mod_products_array_alloc_ = allocate_uint(mod_product_uint64_count_ * mod_count_, pool);
            inv_mod_products_array_alloc_ = allocate_uint(mod_uint64_counts_sum, pool);
            
            // Populate mod_array_anchor_ and mod_array_
            uint64_t *mod_array_ptr = mod_array_alloc_;
            for (int i = 0; i < mod_count_; ++i)
            {
            ...
            Pointer temp(allocate_uint(mod_product_uint64_count_, pool));
            ...
            Pointer big_alloc(allocate_uint(2 * mod_product_uint64_count_, pool));
            ...
            for (int i = 0; i < mod_count_; ++i)
            {
                ...
                divide_uint_uint_inplace(temp.get(), mod_array_[i], mod_product_uint64_count_, mod_products_array_ptr, pool, big_alloc.get());
            ...
            for (int i = 0; i < mod_count_; ++i)
            {
                ...
                modulo_uint_inplace(temp.get(), mod_product_uint64_count_, mod_array_[i], pool, big_alloc.get());

                if (!try_invert_uint_mod(temp.get(), mod_array_[i].get(), mod_uint64_counts_[i], inv_mod_products_array_ptr, pool))
                {
                    reset();
                    return false;
                }
        ...
        void UIntCRTBuilder::reset()
        {
            ...
            if (mod_array_alloc_ != nullptr)
            {
                delete[] mod_array_alloc_;
                mod_array_alloc_ = nullptr;
            }
        ...
        void UIntCRTBuilder::compose(vector<const uint64_t*> inputs, uint64_t *destination) const
        {
            ...
            MemoryPool &pool = *MemoryPool::default_pool();
            Pointer big_alloc(allocate_uint(mod_product_uint64_count_+ 4 * mod_product_uint64_count_, pool));
            for (int i = 0; i < mod_count_; ++i)
            {
                multiply_uint_uint_mod(inputs[i], inv_mod_products_array_[i], mod_array_[i], big_alloc.get(), pool, big_alloc.get() + mod_product_uint64_count_);
...
#endif
            MemoryPool &pool = *MemoryPool::default_pool();
            Pointer big_alloc(allocate_uint(3 * mod_product_uint64_count_, pool));
            for (int i = 0; i < mod_count_; ++i)
            {
                modulo_uint(input, mod_product_uint64_count_, mod_array_[i], destination[i], pool, big_alloc.get());
            }
        }
    }
}